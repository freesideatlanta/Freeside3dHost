...
def main():
	...
	parser.add_option("--tcpCommunication", action="store", type="string", dest="tcpCommunication",
		help="Start commandline TCP monitor")
	...
	if options.tcpCommunication:
		from Cura import tcpCommunication
		ip = options.tcpCommunication
		tcpCommunication.startMonitor(ip)
		return
