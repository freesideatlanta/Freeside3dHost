"""
TCP communication with the printer for printing is done from a separate process,
this to ensure that the PIL does not block the TCP printing.
...
class tcpComm(object):
	"""
	The tcpComm class is the interface class which handles the communication between stdin/stdout and the machineComTcp class.
	This interface class is used to run the (TCP) communication in a different process then the GUI.
	"""
	def __init__(self, ip):
		self._comm = None
		self._gcodeList = []
		self._comm = machineCom.MachineComTCP(ip, callbackObject=self)
