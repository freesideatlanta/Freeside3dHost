...
class tcpConnectionGroup(printerConnectionBase.printerConnectionGroup):
	"""
	The TCP connection group. Keeps track of all saved TCP Connections.
	"""
	def __init__(self):
		super(tcpConnectionGroup, self).__init__("TCP")
		self._connectionMap = {}

	def getAvailableConnections(self):
		#Return the Programmed connection
		#Need MachineCom TCP List
		ip = profile.getMachineSetting('tcp_ip_address');
		
		#Clear all existing cconnections
		for key in self._connectionMap.keys():
				self._connectionMap.pop(key)
		#Add Connection	
		self._connectionMap[ip] = tcpConnection(ip)
...
class tcpConnection(printerConnectionBase.printerConnectionBase):
	"""
	A tcp connection. Needs to build an active-connection.
	When an active connection is created, a 2nd python process is spawned which handles the actual serial communication.

	This class communicates with the Cura.tcpCommunication module trough stdin/stdout pipes.
	"""
	def __init__(self, ip):
		super(tcpConnection, self).__init__(ip)
		self._portName = ip
	...
	def openActiveConnection(self):
		...
		self._thread = threading.Thread(target=self._tcpCommunicationThread)
	...
	def _tcpCommunicationThread(self):
		if platform.system() == "Darwin" and hasattr(sys, 'frozen'):
			cmdList = [os.path.join(os.path.dirname(sys.executable), 'Cura'), '--tcpCommunication']
			cmdList += [self._portName + ':' + profile.getMachineSetting('tcp_ip_address')]
		else:
